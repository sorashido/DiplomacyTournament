		...
		while(System.currentTimeMillis() < negotiationDeadline){
			...
			while(hasMessage()){
				...
				if(receivedMessage.getPerformative().equals(DiplomacyNegoClient.ACCEPT)){
					...
					this.getLogger().logln("ANACExampleNegotiator.negotiate() Received acceptance from " + receivedMessage.getSender() + ": " + acceptedProposal, printToConsole);
					...
					this.getLogger().logln("ANACExampleNegotiator.negotiate() Received proposal: " + receivedProposal, printToConsole);
					...
					if(!outDated && consistencyReport == null){
						...
						if(random.nextInt(2) == 0){ // accept with 50% probability.
							...
							this.getLogger().logln("ANACExampleNegotiator.negotiate()  Accepting: " + receivedProposal, printToConsole);
						}
					}
					...
					this.getLogger().logln("ANACExampleNegotiator.negotiate() RECEIVED CONFIRMATION OF: " + confirmedProposal, printToConsole);
			...
			if(newDealToPropose == null){ //we only make one proposal per round, so we skip this if we have already proposed something.
				...
				if(newDealToPropose != null){
					...
					this.getLogger().logln("ANACExampleNegotiator.negotiate() Proposing: " + newDealToPropose, printToConsole);
